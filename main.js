...
function showSection(sectionId) {
    ...
    updateSiteIndexFooter(sectionId);
...
function buildGameCard(game, sectionFolder) {
    ...
    card.innerHTML = `
        ...
        <div class="card-meta-row">
            ${(game.categories && game.categories.length) ? `<span class="meta-chip">${game.categories[0]}</span>` : ''}
...
// --- فهرس الموقع الزاحف في الأسفل: يتغيّر حسب القسم المفتوح، ويختفي في الأقسام اللي ما تحتاجه ---
function updateSiteIndexFooter(sectionId) {
    const footer = document.querySelector('.site-index-footer');
    const heading = document.getElementById('siteIndexHeading');
    const list = document.getElementById('siteIndexList');
    if (!footer || !list) return;

    if (sectionId === 'games') {
        footer.style.display = 'block';
        if (heading) heading.textContent = 'فهرس جميع التعريبات';
        list.innerHTML = visible(gamesData).map(game =>
            `<li><a href="games/${game.slug}.html">تعريب ${game.title}</a></li>`
        ).join('');
    } else if (sectionId === 'official') {
        footer.style.display = 'block';
        if (heading) heading.textContent = 'فهرس التعريبات الرسمية';
        list.innerHTML = visible(officialGamesData).map(game =>
            `<li><a href="official/${game.slug}.html">${game.title}</a></li>`
        ).join('');
    } else {
...
async function initSite() {
    ...
    renderUpdates();
    renderNews();
    // الفهرس السفلي يُحدَّث تلقائياً داخل showSection() حسب القسم المفتوح
...
// وليس إشعار Push حقيقي يوصل والموقع مسكّر تماماً — هذا يحتاج خدمة خارجية مثل
// OneSignal لأنه GitHub Pages موقع ثابت بدون سيرفر خاص.
// =====================================================================
function setupNotifications() {
    ...
    const dismissed = localStorage.getItem('soim-notify-dismissed') === '1';
    ...
    if (dismissBtn) {
        dismissBtn.addEventListener('click', () => {
            localStorage.setItem('soim-notify-dismissed', '1');
            banner.classList.remove('show');
        });
    }
...
function checkForNewContent() {
    const combined = [
        ...gamesData.map(g => ({ title: g.title, createdAt: g.createdAt || 0 })),
        ...officialGamesData.map(g => ({ title: g.title, createdAt: g.createdAt || 0 })),
        ...newsData.map(n => ({ title: n.title, createdAt: n.createdAt || 0 }))
    ];
